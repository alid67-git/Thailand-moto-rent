...
import { getAllArticleIds, getArticleBody } from "@/lib/travel-guide-content";
...
export async function generateMetadata({
  ...
  return {
    title: `${article.title} | Seyahat Rehberi`,
...
export default async function TravelGuideArticlePage({
  ...
  const { id } = await params;
  const article = TRAVEL_GUIDE_ARTICLES.find((a) => a.id === id);
  const content = getArticleBody(id);

  if (!article || !content) {
    notFound();
  }

  const related = TRAVEL_GUIDE_ARTICLES.filter((a) => a.id !== id && a.category === article.category)
    .slice(0, 3)
    .concat(TRAVEL_GUIDE_ARTICLES.filter((a) => a.id !== id && a.category !== article.category).slice(0, 1))
    .slice(0, 3);
      ...
      <section className="panel-section bg-surface">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="panel-accent mt-12 p-8 text-center">
            ...
            <p className="mt-2 text-body">IDP ve sigortanızı tamamlayıp bugün rezervasyon yapın.</p>
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-3">
            {[
              ...
              { href: "/destinations", label: "Gezilecek yerler" },
              { href: "/motorcycles", label: "Motosikletler" },
            ].map((link) => (
              <Link key={link.href} href={link.href} className="panel flex items-center justify-between p-4 transition hover:border-brand-500">
          ...
          {related.length > 0 && (
            <div className="mt-12 border-t border-neutral-200 pt-8 dark:border-ink-700">
              <h3 className="font-heading text-lg font-bold text-ink-950 dark:text-neutral-50">İlgili makaleler</h3>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {related.map((art) => (
                  <Link key={art.id} href={`/travel-guide/${art.id}`} className="panel p-4 transition hover:border-brand-500">
                    <h4 className="font-bold text-ink-950 dark:text-neutral-100 line-clamp-2">{art.title}</h4>
