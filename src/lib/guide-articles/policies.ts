...
export const GUIDE_POLICIES: Record<Locale, string> = {
  tr: `
...
- **IDP (Uluslararası Sürücü Belgesi) zorunludur.** Tayland polisi yabancılar için **Tayland ehliyeti veya IDP dışında hiçbir belgeyi geçerli saymaz**.
...
- **Kiralama gün bazlıdır.** Geç teslimde mutlaka **WhatsApp ile haber verin**.
- **Test sürüşü yoktur.**
...
- **IDP (International Driving Permit) is mandatory.** Thai police accept only a **Thai licence or IDP** for foreigners.
...
`,
  de: `
## Thailand Moto Rent Richtlinien
...
`,
  ru: `
## Политика Thailand Moto Rent

- **Паспорта не держим.** **Предавторизация по кредитной карте** как депозит; снимается при возврате байка.
- **IDP обязателен.** Тайская полиция принимает только **тайские права или IDP**.
- **Условия аренды:** действующий **мото-IDP**; без IDP аренда невозможна.
- **Езда без IDP** и штрафы — полностью на клиенте.
- **Посуточная аренда.** При опоздании сообщите в **WhatsApp**.
- **Тест-драйвов нет.**
- **Повреждения:** без страховки = ремонт + потерянные дни аренды. Рекомендуем **полное покрытие**.
- **Пьяная езда** — тяжёлые штрафы.
`,
  zh: `
